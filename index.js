  ...
  try {
    ...
    app.post("/jwt", (req, res) => {
      ...
      const token = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, {
        expiresIn: "1h",
      }); // Assigning JWT token first time after login. Expire time is in ms.
    ...
    app.get("/orders", verifyJWT, async (req, res) => {
      ...
      if (decoded.email !== req.query.email) {
        // check if decoded email and login user email are same.
    ...
    app.post("/orders", async (req, res) => {
      ...
      const { serviceName, customerName, email, address, postcode, currency } =
        order;
    ...
    app.post("/payment/success", async (req, res) => {
      ...
      const result = await orderCollection.updateOne(
        { transactionId },
        { $set: { paid: true, paidAt: new Date().toLocaleString() } }
      );
